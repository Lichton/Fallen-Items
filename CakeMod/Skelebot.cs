...
namespace CakeMod
{
	...
	public class Skelebot : CompanionItem
	{
		//Add Snowballs and Sunglasses synergy
		public static void Init()
		{
			string name = "Skelebot";
			string resourcePath = "CakeMod/Resources/Skelebot";
			GameObject gameObject = new GameObject();
			var penguin = gameObject.AddComponent<Skelebot>();

			ItemBuilder.AddSpriteToObject(name, resourcePath, gameObject);

			string shortDesc = "Termination Time";
			string longDesc = "A broken machine lost to the Gungeon.\n" +
				"In it's fractured memory banks, it remembers a battle against a crusader.";
			ItemBuilder.SetupItem(penguin, shortDesc, longDesc, "cak");
			penguin.quality = PickupObject.ItemQuality.EXCLUDED;
			penguin.CompanionGuid = Skelebot.guid;
			penguin.Synergies = new CompanionTransformSynergy[0];
			Skelebot.BuildPrefab();
			CakeIDs.SkelebotID = penguin.PickupObjectId;
		}
		...
		private void YourMother2(Vector2 obj)
		{
			PlayerController player = (GameManager.Instance.PrimaryPlayer);
			LootEngine.GivePrefabToPlayer(PickupObjectDatabase.GetById(314).gameObject, player);
		}
		...
		private static void YourMother(Vector2 obj)
		{
			PlayerController player = (GameManager.Instance.PrimaryPlayer);
			LootEngine.GivePrefabToPlayer(PickupObjectDatabase.GetById(314).gameObject, player);
			
			PenguinPrefab.gameObject.AddComponent<Stickyclass>();
		
			player.RemovePassiveItem(CakeIDs.SkelebotID);
		}

		private class Stickyclass : MonoBehaviour
		{
			private void Start()
			{
				base.gameObject.GetComponent<HealthHaver>().OnPreDeath += Ded;

			}

			private void Ded(Vector2 obj)
			{
				Instantiate(EasyVFXDatabase.ChestTeleporterTimeWarp, base.gameObject.GetComponent<AIActor>().CenterPosition, Quaternion.identity);
			}
		}
		...
		public static readonly string guid = "skelebot";
		...
		{
			"CakeMod/Resources/skelebot/skelebot_left_001",
			"CakeMod/Resources/skelebot/skelebot_left_002",
			"CakeMod/Resources/skelebot/skelebot_right_001",
			"CakeMod/Resources/skelebot/skelebot_right_002"
		};
		...
		public class PenguinBehavior : CompanionController
		{
			...
			private void Start()
			{
				...
				base.aiActor.healthHaver.OnPreDeath += YourMother;
				base.aiActor.healthHaver.OnPreDeath += YourMOther3;
				this.Owner = this.m_owner;
			}
			private void YourMOther3(Vector2 obj)
			{
				Instantiate(EasyVFXDatabase.ChestTeleporterTimeWarp, base.aiActor.sprite.WorldCenter, Quaternion.identity);
			}
		...
		public class PenguinAttackBehavior : AttackBehaviorBase
		{
			...
			public override void Init(GameObject gameObject, AIActor aiActor, AIShooter aiShooter)
			{
				...
				this.Owner = this.m_aiActor.GetComponent<Skelebot.PenguinBehavior>().Owner;
			}
					...
					if (flag3)
					{
						...
						if (flag4)
						{
							...
							if (flag5)
							{
								...
								Projectile projectile = ((Gun)ETGMod.Databases.Items[45]).DefaultModule.projectiles[0];
