...
namespace CakeMod
{
	...
	internal class ControlledBullets : GunVolleyModificationItem
	{
		// Token: 0x0600008B RID: 139 RVA: 0x00005B8C File Offset: 0x00003D8C
		public static void Init()
		{
			string name = "Controlled Bullets";
			string resourcePath = "CakeMod/Resources/ControlledBullets";
			GameObject gameObject = new GameObject(name);
			ControlledBullets catSnack = gameObject.AddComponent<ControlledBullets>();
			ItemBuilder.AddSpriteToObject(name, resourcePath, gameObject);
			string shortDesc = "Mind Controll";
			string longDesc = "These bullets give a chance upon firing to control the enemy fired at.\n" +
				"These bullets originally were a special version of the 3rd party controller but upon their introduction to the gungeon the gungeons magic changed them into actual bullets.";
		...
		private void PostProcessProjectile(Projectile projectile, float Chance)
		{
			...
			bool flag4 = flag3 && !ControlledBullets.CoolAsIce;
			if (flag4)
			{
				ControlledBullets.CoolAsIce = true;
				base.StartCoroutine(this.StartCooldown());
				float num = 0.97f - statValue / 50f;
				bool flag5 = UnityEngine.Random.value > num;
				bool flag6 = flag5 && !ControlledBullets.SharkMFs;
				if (flag6)
				{
					ControlledBullets.SharkMFs = true;
					base.StartCoroutine(this.SharkCooldown());
					Projectile projectile2 = ((Gun)ETGMod.Databases.Items[511]).DefaultModule.chargeProjectiles[0].Projectile;
				...
				{
					Projectile projectile3 = ((Gun)ETGMod.Databases.Items[511]).DefaultModule.projectiles[0];
		...
		private void PostProcessBeamChanceTick(BeamController beamController)
		{
			...
			if (flag4)
			{
				...
				if (flag6)
				{
					ControlledBullets.SharkMFs = true;
					base.StartCoroutine(this.SharkCooldown());
					Projectile projectile = ((Gun)ETGMod.Databases.Items[153]).DefaultModule.chargeProjectiles[0].Projectile;
				...
				{
					Projectile projectile2 = ((Gun)ETGMod.Databases.Items[7]).DefaultModule.projectiles[0];
		...
		private IEnumerator StartCooldown()
		{
			...
			ControlledBullets.CoolAsIce = false;
			yield break;
		}
		...
		private IEnumerator SharkCooldown()
		{
			...
			ControlledBullets.SharkMFs = false;
			yield break;
		}
